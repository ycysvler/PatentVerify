...
import $ from 'jquery';
import {FastActions, FastStore} from '../fast/fastApi.js';

const {Content} = Layout;

class AttachedPartCreate extends React.Component {
    constructor(props) {
        super(props);
    ...
    inputChange() {
        ...
        FastActions.uploadImage(Config.url + "/uploadimages.ashx?username=" + this.getCookie("user_name"), data, filenames, this.getCookie("token"));
    }
    ...
    renderOneImage(url) {
        return <div>
            <img alt="" style={{maxWidth:500, maxHeight:500}} src={Config.url + "/" + url}/>
        </div>
    }
                                            ...
                                            return <div key={image}  style={{ height: 50, width:50 ,marginRight:8,  float:'left'}}>
                                                <Popover  content={self.renderOneImage(image)}>
                                                    <img alt="" style={{width: "90%", height: "90%"}}
                                                         src={Config.url + "/" + image}/></Popover></div>
                                        })
                                    }
...
export default AttachedPartCreate;
